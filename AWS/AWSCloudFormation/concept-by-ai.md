...
# AWS CloudFormationとは 要約

- AWSのインフラ（サーバー、DB、ネットワーク等）をYAML/JSONのテンプレートで自動構築・管理できるサービス（IaC）。
- テンプレートをアップロードして「スタック」としてまとめてデプロイ・削除できる。
- 依存関係も自動で処理され、再現性・自動化・変更追跡・CI/CD連携が可能。
- 例：S3バケットなどもテンプレート1つで自動作成できる。
- 主な用途：環境の複製、CI/CD、サーバーレス、ネットワーク自動設定など。
- 他のIaCツール（Terraform, CDK等）との違い：AWS公式でAWSに最適化されている。
