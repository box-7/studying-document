...
# AWS CloudFormation テンプレートリファレンス 要約

このリファレンスは、CloudFormationテンプレート作成時に使える各種コンポーネントの詳細情報をまとめたものです。  
主な内容は以下の通りです。

- **リソースタイプとプロパティ**：作成・管理できるAWSリソースの種類と設定項目
- **リソース属性**：リソースの動作や連携を制御する特別な設定
- **組み込み関数**：テンプレート内で使える便利な関数
- **トランスフォーム**：テンプレート内で特定の処理を簡単に実行するマクロ
- **ヘルパースクリプト**：EC2インスタンスのセットアップ用スクリプト
- **リソース仕様とスキーマ**：各リソースタイプの詳細仕様

CloudFormationを初めて使う場合は、公式ユーザーガイドの「テンプレートの使用」や「使用開始」セクションを参照することが推奨されています。
