...
# AWS CloudFormation とは 要約

- AWSリソース（EC2やRDSなど）をテンプレートで記述し、自動でプロビジョニング・設定できるサービス
- 個別リソースの作成や依存関係の管理が不要になり、インフラ管理が簡単に
- テンプレートを使えば、複数リージョンへのリソース複製や一括削除も容易
- テンプレートはテキストファイルなので、バージョン管理や変更履歴の追跡が可能
- 利用方法はコンソール、API、CLI、SDKなど多様
